...
enum packet_type get_pkt_type(rel_t* r, packet_t *pkt, size_t n);
...
void rel_recvpkt(rel_t *r, packet_t *pkt, size_t n) {
  ...
  enum packet_type pkt_type = get_pkt_type(r, pkt, n);
  pkt_ntoh(pkt);
  // if(pkt->len == 8){
  //   print_pkt(pkt, "receive ack", pkt->len);
  // }else{
  //   print_pkt(pkt, "receive data", pkt->len);
  // }
...
void process_ack_pkt(rel_t *r, packet_t *pkt) {
  ...
  pkt_hton(pkt);
  print_pkt(pkt, "receive ack", ntohs(pkt->len));
  pkt_ntoh(pkt);
...
void process_data_pkt(rel_t *r, packet_t *pkt) {
  ...
  pkt_hton(pkt);
  print_pkt(pkt, "receive data", ntohs(pkt->len));
  pkt_ntoh(pkt);
...
void process_eof_pkt(rel_t *r, packet_t *pkt) {
  ...
  pkt_hton(pkt);
  print_pkt(pkt, "receive EOF", ntohs(pkt->len));
  pkt_ntoh(pkt);
...
uint16_t output_pkt(rel_t *r, packet_t *pkt, uint16_t start, uint16_t payload_len) {
  ...
  uint16_t bytes_to_output = __builtin_fmin(bufspace, payload_len - start);
...
int send_ack_pkt(rel_t *r, uint32_t ackno) {
  ...
  char* buf = malloc(DATA_PACKET_MAX_PAYLOAD_LEN*sizeof(char));
  int r1 = conn_input(r->c, buf, sizeof(buf));
  fprintf(stderr, "%d, READ len = %d\n", getpid(), r1);
  if(r1 == 0){
    init_ack_pkt(&pkt,ackno);
  }
  else if(r1 != -1){
    init_data_pkt(&pkt,ackno,r->last_seqno_sent+1,buf,strlen(buf));
  }
  ...
  print_pkt(&pkt, "send ack", ntohs(pkt.len));
  pkt_ntoh(&pkt);
  if (bytes_sent > 0) {
...
int send_data_pkt(rel_t *r, packet_t *pkt) {
  ...
  if(r->read_eof){
    print_pkt(pkt, "send EOF", ntohs(pkt_len));
  }
  else{
    print_pkt(pkt, "send data", ntohs(pkt_len));
  }
